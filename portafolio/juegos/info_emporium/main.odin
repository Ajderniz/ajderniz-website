/*******************************************************************************
*
* "The Information Emporium" - by Axel "Ajderniz" Lopez
*
* An extremely nonsensical, annoying and time-consuming endeavour, that I'm
* proud of, by virtue of it being the only game I've managed to complete and
* publish.
*
* This game was made for the O2A2 VN Jam 2025, a micro Visual Novel jam with
* strict asset limitations and a 1000 word limit. 
*
*******************************************************************************/

package info_emporium
...
main :: proc()
{
  ...
  // Contains keys that have been "found" by the player
  key_set: bit_set[Key] = { .NONE }

  // Lil' buffer used to keep a 'log' of previous interactions
  lineopt_buffer: [3]LineOpt = { { -1, -1 }, { -1, -1 }, { -1, -1 } }
  ...
  // virtual fps: used for things that one may want to feature a lower framerate
  vfps := 15
  vfps_freq := fps / vfps
  vfps_counter := 0

  font16 := rl.LoadFontEx("./res/fonts/DOS-V_re_ANK16.ttf", 16, nil, 0)
  font30 := rl.LoadFontEx("./res/fonts/DOS-V_re_ANK30.ttf", 30, nil, 0)

  bg_txr := rl.LoadTexture("./res/img/bg.png")
  fg_txr := rl.LoadTexture("./res/img/fg.png")
  face_txrs: [Face]rl.Texture =
  {
    .NEUTRAL = rl.LoadTexture("./res/img/face/neutral.png"),
    .ATTENTIVE = rl.LoadTexture("./res/img/face/attentive.png"),
    .INDIFFERENT = rl.LoadTexture("./res/img/face/indifferent.png"),
    .ANNOYED = rl.LoadTexture("./res/img/face/annoyed.png"),
    .ANGRY = rl.LoadTexture("./res/img/face/angry.png"),
    .SOLEMN = rl.LoadTexture("./res/img/face/solemn.png"),
    .FLUSTERED = rl.LoadTexture("./res/img/face/flustered.png"),
    .HAPPY = rl.LoadTexture("./res/img/face/happy.png"),
    .ECSTATIC = rl.LoadTexture("./res/img/face/ecstatic.png")
  }

  // Audio initialization
  rl.InitAudioDevice()
  rl.SetAudioStreamBufferSizeDefault(4096) // This helps with audio crackling

  sound := rl.LoadSound("./res/sound/sound.wav")
  music := rl.LoadMusicStream("./res/sound/gnossienne1.ogg")
  rl.SetSoundVolume(sound, 0.5)

  // Values used for the 'draw_text_box' function
  option_indent ::    12 // Indentation that helps tell options and lines apart
  text_box_padding :: 12 
  text_padding ::     4
  // Tese are used to keep track of the option buttons for mouse input
  options_y: [2]f32
  options_h: [2]f32
  options_hover: [2]b8 = false
  // This would normally be calculated on-the-fly for a dynamically-resized text
  // box, but since these are fixed in size in this game, might as well...
  ...
  screen_state: enum{ TITLE, INTRO, GAME } = .TITLE

  is_typing: b8 = true    // Whether the letter-by-letter effect is active
  typed_characters := 1   // Amount of typed characters
  ...
  for false == rl.WindowShouldClose()
  {
    ...
    case .GAME:
      ...
      rl.BeginDrawing()
        ...
        // - y_increment is set to the height of the drawn box;
        //    this helps render the boxes below it
        y_increment: f32 = draw_text_box(
                        font16, 
                        text, 
                        rl.WHITE,
                        { 0, 0 }, 
        ...
        options_y[0] = y_increment + text_box_padding
        ...
        if false == is_typing
        {
          ...
          {
            rect_color := rl.WHITE - { 0, 0, 0, 64 } if false == options_hover[0] else rl.WHITE
            rl.DrawTextEx(
              font16,
              "1.",
              { 2, y_increment + text_box_padding * 2 },
              cast(f32)font16.baseSize,
              0,
              rl.YELLOW)
            options_h[0] = draw_text_box(
                            font16,
                            STR_CONTINUE,
                            rl.WHITE,
                            { option_indent, options_y[0] },
          ...
          else // If the current line has options, draw each
          {
            y := options_y[0]
            i: i8 = 1
            for o, i in line.options
            {
              if 0 <= o
              {
                ...
                options_h[i] = y_increment
                y += y_increment + text_box_padding
              }
            }
            options_y[1] = options_y[0] + options_h[0] + text_box_padding
          }
        }
        ...
        for lo in lineopt_buffer
        {
          ...
          y_increment = draw_text_box(
            ...
            text_padding,
            font16_max_col_width,
            key_colors[options[lo.option].key] - { 0, 0, 0, 192 }) // Fade color
          y += y_increment + text_box_padding
          ...
          y_increment = draw_text_box(
            ...
            text_padding,
            font16_max_col_width,
            key_colors[lines[lo.line].key] - { 0, 0, 0, 192 }) // Fade color
          y += y_increment + text_box_padding
        }
  ...
  // De-initializatin and unloading ============================================
